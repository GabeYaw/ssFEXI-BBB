...
tm = (
    np.array([20, 20, 20, 20, 200, 200, 400, 400], dtype=np.float32) * 1e-3
)  # mixing time [s]
...
sim_adc = (
    feeq * De + fieq * Di
)  # ADC, simulated [um2/ms] (I think units are the same because it is a weighted sum)

# s0 = 1 at this point, so don't need to divide by anything
sbf_s0 = (1 - fieq) * np.exp(-bf * De) + fieq * np.exp(
    -bf * Di
)  # s(bf), simulated [a.u]
...
if noise == 1:
    ...
    Sim_E_vox_real = sim_E_vox + np.random.normal(
        scale=1 / 50, size=np.shape(sim_E_vox)
    )  # adding rician noise, snr = 50
